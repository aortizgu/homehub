...
ExteriorTempProvider::ExteriorTempProvider() {
    loc = app().getCustomConfig()["controller"]["openweather-loc"].asString();
    api = app().getCustomConfig()["controller"]["openweather-api"].asString();
}
            ...
            [cb](ReqResult result, const HttpResponsePtr &response) {
                ...
                if (result == ReqResult::Ok) {
                    try {
                        ...
                        spdlog::error("ExteriorTempProvider::getExteriorTemp: error parsing response");
                    }
                }
                spdlog::debug(
