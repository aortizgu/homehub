...
bool DigestAuthFilter::isEndOfAttributeName(size_t pos, size_t len,
        const char *data) {
    if (pos >= len)
        return true;
...
void DigestAuthFilter::httpParseAttributes(const char *data, size_t len,
...
void DigestAuthFilter::doFilter(const HttpRequestPtr &req, FilterCallback &&cb,
    ...
    if (!auth_header.empty()) {
        ...
        if (httpHasAttribute(att_list, "username", &username)
            ...
            if (credentials.find(username) != credentials.end()) {
                std::string A1 = username + ":" + realm + ":"
                        + credentials.at(username);
                std::string A2 = method2String(req->getMethod()) + ":" + uri;
                std::string A1_middle_A2 = toLower(utils::getMd5(A1)) + ":"
                        + nonce + ":" + toLower(utils::getMd5(A2));
                std::string calculated_response = toLower(
                        utils::getMd5(A1_middle_A2));
